...
class News extends Equatable {
  ...
  static News fromRss(XmlElement item) {
    var unescape = new HtmlUnescape();

    return News(
      authors: [unescape.convert(item
          .getElement('dc:creator')
          ?.text ?? "")
      ],
      title: unescape.convert(item
          .getElement('title')
          ?.text ?? ""),
      updated: parseRfc822(item
          .getElement('pubDate')
          ?.text ?? "0"),
      content: unescape.convert(item
          .getElement('content:encoded')
          ?.text ?? ""),
      summary: unescape.convert(item
